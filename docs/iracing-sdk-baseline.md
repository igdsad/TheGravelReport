...
A positive `WeekendInfo:SubSessionID` creates durable event key `v2:event:subsession:{id}`. `SessionNum` remains part of every replay position but is not part of that event identity, so practice, qualifying, and heats in one SubSession share one incident list. A changed session number creates a new counter epoch and baseline without fake points. Missing, zero, malformed, misplaced, or duplicate evidence produces a UUIDv7 connection-scoped key; that key also groups heat numbers for the life of the logical SDK connection but cannot survive a process restart. Existing `v1` per-heat database rows remain valid and separate; the adapter does not heuristically merge or rewrite them. The narrow parser recognizes indentation, quoted scalar values, escapes while finding delimiters, and comments; it does not pretend to be a general YAML implementation.
