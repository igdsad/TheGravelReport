...
| Eligible participant scored counters | `DriverInfo:Drivers[]:TeamIncidentCount` from current session information |
...
| Safe local substitute | `PlayerCarTeamIncidentCount`, only when the matching current `DriverInfo` row supplies the same deterministic participant identity but lacks a usable row-level counter |
...
An eligible field entry has a unique supported `CarIdx`, a deterministic canonical identity from valid current `TeamID`/`UserID` evidence, a valid non-negative `TeamIncidentCount`, is not marked `CarIsPaceCar` or `IsSpectator`, and is not the `DriverInfo:PaceCarIdx`. An omitted, hidden, or unusable entry contributes no participant observation; it is never converted to a zero counter. `SessionInfo:CurrentSessionNum` is required to match the live telemetry frame's `SessionNum` before any row is authoritative. A valid frame may therefore contain an empty participant-counter collection while still carrying connection, replay position, on-track state, and observation time.

The adapter derives identity independently on every frame: `car-index:{CarIdx}:team:{TeamID}` when `TeamID` is positive, otherwise `car-index:{CarIdx}:user:{UserID}` only when the row explicitly supplies a non-positive team value and a positive user value. Missing, malformed, or insufficient identity evidence omits that row rather than inventing a weaker identity or consulting process memory. Changed positive canonical evidence creates a different participant identity. Exact reuse of the same `CarIdx` and canonical ID after an intervening entrant is indistinguishable in the SDK evidence and therefore reuses the earlier checkpoint; differentiating that A→B→A case would require another durable simulator identifier or persisted generation evidence. For a team car, a driver-only `UserID`/display change leaves the positive team identity and cumulative `TeamIncidentCount` unchanged. The driver name is observation context, not evidence that the named driver caused the points. Display `CarNumber` is not used for identity or replay command packing; focus uses the exact matching entry's validated `CarNumberRaw`.

The application persists one checkpoint per session and deterministic participant identity. Temporary participant or whole-roster omission leaves existing checkpoints untouched. The adapter never reads `PlayerCarMyIncidentCount`, whose personal semantics differ from the row-level team/car counter. It uses `PlayerCarTeamIncidentCount` only when current-session metadata still contains the matching player row and canonical identity but that row lacks a usable `TeamIncidentCount`; otherwise it emits no local observation. This rule is identical after process restart. On authoritative recovery, equality means no change, an increase records one marker with the complete positive delta accumulated across the gap, and a decrease is a reset that advances only that participant's epoch and establishes a new baseline without an incident. Multiple participant increases in one published sample produce separate markers. A multi-point jump for one participant produces one delta marker because the samples contain no trustworthy timing for intermediate values.
...
The camera adapter parses only the required official session-information shape: eligible `DriverInfo:Drivers` focus entries and their freshly derived deterministic identity, `CarIdx`, and `CarNumberRaw`; the local `DriverInfo:DriverCarIdx` context retained only for incidents migrated under the legacy `local-player` identity; and `CameraInfo:Groups` with each `GroupNum`, `GroupName`, and ordered `CameraNum` value. Focus is permitted only when metadata `CurrentSessionNum` matches both the frame's live `SessionNum` and active `ReplaySessionNum`, then the stored identity must exactly match one current row; there is no process-memory rewrite or car-index fallback. A non-null preference is matched case-insensitively to a group and uses its first camera for the command. A null preference reuses the currently reported group/camera. Confirmation revalidates those three session numbers and the exact participant identity/raw car number in the later frame before requiring `CamCarIdx` and `CamGroupNumber`; it intentionally does not require the exact `CamCameraNumber` because a TV group can advance shots automatically. Different current identity evidence, an absent/ambiguous participant, or missing/malformed/unrepresentable session, focus, or camera metadata fails closed before broadcasting or prevents confirmation rather than focusing a replacement.
...
- camera focus revalidates the session and exact recorded-participant identity/raw car number, then confirms its car index and resolved camera group;
...
`IncidentReview.Iracing.ProtocolSimulator` is an independent executable: it has no production-project reference and independently writes the pinned header, variable headers, UTF-8 and ISO-8859-1 participant-bearing session text, three rotating variable buffers, tick transitions, and named event. Integration tests start it out of process with unique kernel-object names and cover encoding negotiation, metadata/session-version caching, a session update racing a cache-miss copy, current-buffer rotation, tick regression/resynchronization, live/moving-replay/paused-session-screen decoding, fractional-millisecond flooring, eligible participant filtering and display/identity extraction, deterministic identity omission/change/recreation behavior, opponent-only counter publication, roster-reorder coalescing, empty-counter samples for mismatched or insufficient metadata, safe local `PlayerCarTeamIncidentCount` substitution without personal-scalar mixing, durable and connection-scoped session identity, diagnostic coalescing and rearming, atomic replay recovery, recovery from malformed Boolean/session text/metadata/time without provisional-key rotation, persistent invalid input ending the logical connection only at the last-successfully-decoded-sample deadline, a valid frame at the exact deadline starting a new logical connection, coalesced valid frames refreshing that deadline, true disconnect/reconnect key rotation, caller cancellation, producer-joined disposal, slow-consumer reset/rise preservation, deterministic bounded-buffer overflow, and confirmation of an outstanding seek by a position-only frame that the application event queue may coalesce. A focused ACL test proves the adapter can open and release an event whose DACL grants the current consumer only `SYNCHRONIZE`.

Replay-controller tests compare seek, camera, and playback payloads with independently authored raw 32-bit golden vectors. They cover representability, delivery branches, strict participant/camera metadata parsing, migrated-local and opponent participant focus, exact canonical identity lookup, explicit identity change, live/replay session-metadata mismatch refusal, preferred/current camera resolution, later-frame requirements, seek tolerance, TV-shot camera-number changes, cancellation, disconnect, and the three distinct confirmation timeouts. A Windows-only integration test sends the production seek command through `RegisterWindowMessageW`/`SendNotifyMessageW` and proves that a hidden native top-level receiver gets the exact `wParam` and `lParam`. The native receiver and independent shared-memory simulator remain separate test peers; no current automated test drives the packaged WPF application through all three confirmed commands, and none substitutes for live-iRacing acceptance.
...
- set **Max Cars = 63** and validate which eligible `Drivers[].TeamIncidentCount` entries are exposed in solo, team, hosted, admin, and broadcaster contexts;
- validate driver swaps, temporary identifier loss/recovery, explicit identity changes, exact-canonical slot-reuse ambiguity, participant and whole-roster omission/reappearance, local team-scalar substitution and abstention without personal-scalar mixing, restart behavior, counter resets, same-update multi-participant changes, batched deltas, and the expected inability to observe `0x` contacts;
...
- prove recorded-participant focus for local, opponent, and team-car incidents, including the expected structured failure when the recorded participant is absent from current metadata;
